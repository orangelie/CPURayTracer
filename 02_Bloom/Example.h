...
public:
	Example(HWND window, int width, int height)
	{
		...
		{
			image.pixels[i].v[0] = std::clamp(image.pixels[i].v[0] * 0.5f, 0.0f, 1.0f);
			image.pixels[i].v[1] = std::clamp(image.pixels[i].v[1] * 0.5f, 0.0f, 1.0f);
			image.pixels[i].v[2] = std::clamp(image.pixels[i].v[2] * 0.5f, 0.0f, 1.0f);
		}
		*/
		...
		*/
		...
		for (int i = 0; i < image.height; ++i)
		{
			for (int j = 0; j < image.width; ++j)
			{
				...
				if (j < image.width / 2.0f)
				{
					image.pixels[index].v[0] = std::clamp(image.pixels[index].v[0] * 0.95f, 0.0f, 1.0f);
					image.pixels[index].v[1] = std::clamp(image.pixels[index].v[1] * 0.95f, 0.0f, 1.0f);
					image.pixels[index].v[2] = std::clamp(image.pixels[index].v[2] * 0.95f, 0.0f, 1.0f);
				}
				else
				{
					image.pixels[index].v[0] = std::clamp(image.pixels[index].v[0] * 1.05f + 0.001f, 0.0f, 1.0f);
					image.pixels[index].v[1] = std::clamp(image.pixels[index].v[1] * 1.05f + 0.001f, 0.0f, 1.0f);
					image.pixels[index].v[2] = std::clamp(image.pixels[index].v[2] * 1.05f + 0.001f, 0.0f, 1.0f);
				}
			}
		}
		*/
